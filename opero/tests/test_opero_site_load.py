"""Load opero-content Markdown into Site DocTypes."""
...
class TestSiteLoad(FrappeTestCase):
	def setUp(self):
	...
	def test_load_maps_content_files_and_ignores_home_team(self):
		...
		self.assertEqual(len(frappe.get_all("Site Team Member")), 1)
		...
		publication = frappe.get_doc("Site Publication", "january-2025-update")
		...
		member = frappe.get_doc("Site Team Member", "anita-onyango")
	...
	def test_load_keeps_extra_local_team_members(self):
		...
		names = set(frappe.get_all("Site Team Member", pluck="name"))
