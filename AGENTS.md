...
Opero Site DocTypes load and publish Markdown in `opero-con/opero-content`. Set `opero_content_github_token` in `site_config.json` (optional `opero_content_repo`, `opero_content_base_branch`). Desk sidebar: Opero Website. Deploy Center lists files due for deploy and the last ten GitHub commit links; Deploy to website lives there. Load from website content stays on Site Settings. Load upserts by slug and does not delete extra local team members or enterprises. Cubenet uses Show on website on Publication, Team Member, and Enterprise; Status is the title pill, not a form field (Enterprise CRM lifecycle stays on `enterprise_status`). Home Page, Privacy policy, and Site Settings are always on-site and have no checkbox. Checking queues To deploy; already-Published optional records stay Published until unchecked. Always-on singles flip back to To deploy on Desk save so the pill matches Deploy Center. Unchecking a live record queues To unpublish; a never-live record goes back to Draft. After Deploy to website succeeds (or finds the public site already up to date), To deploy becomes Published and To unpublish becomes Unpublished. Drafts keep the live GitHub file unchanged. Load from GitHub marks live files Published and inactive team members / enterprises Unpublished. New publications, team members, and enterprises start as Draft so Cubenet can edit without pushing. Deploy commits to `main`; cubenet is the approval. Token needs Contents read/write on `opero-content` (no Pull requests). Do not put the token in git.
