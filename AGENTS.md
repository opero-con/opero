...
Opero Site DocTypes load and publish Markdown in `opero-con/opero-content`. Set `opero_content_github_token` in `site_config.json` (optional `opero_content_repo`, `opero_content_base_branch`). Desk sidebar: Opero Website. Deploy Center lists files due for deploy and the last ten GitHub commit links; Deploy to website lives there. Load from website content stays on Site Settings. Load upserts by slug and does not delete extra employees or enterprises. Cubenet uses the Publish checkbox (`show_on_website`) on Publication, Employee, Enterprise, and Partner; website status is the title pill, not a form field (Employee HR status and Enterprise CRM lifecycle remain separate). Home Page, Privacy policy, and Site Settings are always on-site and have no checkbox. Statuses: Draft (not on the site: never live, or taken down), To publish, To update, Published, To unpublish. Ticking Publish queues To publish, or To update when the record is already live (including re-ticking To unpublish). Unticking a live record queues To unpublish; otherwise it returns to Draft. Always-on singles go to To update on Desk save so the pill matches Deploy Center. After Deploy to website succeeds (or finds the public site already up to date), To publish and To update become Published and To unpublish becomes Draft. Drafts keep the live GitHub file unchanged. Load from GitHub marks live files Published and inactive employees, enterprises, and partners Draft. New publications, employees, enterprises, and partners start as Draft so Cubenet can edit without pushing. Employee website profiles use the standard Employee image by default and can opt into a separate portrait. Deploy commits to `main`; cubenet is the approval. Token needs Contents read/write on `opero-content` (no Pull requests). Do not put the token in git.
